...
std::string to_string(const weekdays::type& val) {
  std::map<int, const char*>::const_iterator it = _weekdays_VALUES_TO_NAMES.find(val);
  if (it != _weekdays_VALUES_TO_NAMES.end()) {
...
foo::foo(const foo& other0) noexcept {
  bar = other0.bar;
  baz = other0.baz;
  qux = other0.qux;
  bop = other0.bop;
...
foo& foo::operator=(const foo& other1) noexcept {
  bar = other1.bar;
  baz = other1.baz;
  qux = other1.qux;
  bop = other1.bop;
...
bool foo_error::operator==(const foo_error & rhs) const
{
...
const char* foo_error::what() const noexcept {
  try {
    std::stringstream ss;
...
ostr_default::~ostr_default() noexcept {
}
...
bool ostr_default::operator==(const ostr_default & rhs) const
{
  if (!(bar == rhs.bar))
    return false;
  return true;
...
ostr_default::ostr_default(const ostr_default& other4) noexcept {
  bar = other4.bar;
  __isset = other4.__isset;
}
ostr_default& ostr_default::operator=(const ostr_default& other5) noexcept {
  bar = other5.bar;
...
ostr_custom::~ostr_custom() noexcept {
}
...
bool ostr_custom::operator==(const ostr_custom & rhs) const
{
  if (!(bar == rhs.bar))
    return false;
  return true;
...
ostr_custom::ostr_custom(const ostr_custom& other6) noexcept {
  bar = other6.bar;
  __isset = other6.__isset;
}
ostr_custom& ostr_custom::operator=(const ostr_custom& other7) noexcept {
  bar = other7.bar;
